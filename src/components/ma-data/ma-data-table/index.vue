<template>
	<div class="ma-data-table">
		<el-dialog :model-value="visible" :title="props.title" :close-on-click-modal="false" :width="800" draggable @close="handleClose">
			<el-form v-if="queryParams.length > 0" :inline="true" :model="state.queryForm" style="margin-bottom: 15px">
				<el-form-item v-for="(query, index) in queryParams" :key="index">
					<el-input v-model="state.queryForm[query.field]" :placeholder="query.label" />
				</el-form-item>
				<el-form-item>
			...
			>
				<el-table-column v-if="multiple" type="selection"  align="center" width="50" />
